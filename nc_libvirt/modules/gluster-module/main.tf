...
resource "libvirt_volume" "data_disk" {
  for_each = local.servers
  name           = "data-disk-${var.machine_name}${each.value["index"]}-xfs"
  pool           = "default"
  size           = var.diskBytes
  format         = "qcow2"
}

resource "libvirt_domain" "gluster_vms" {
for_each = local.servers
  name       = each.key
  ...
  dynamic "disk" {
    for_each = each.value["dd"] ? [libvirt_volume.data_disk.*] : []
    content {
#        volume_id = "${each.key}-disk-${disk.key}"
        volume_id = "/var/lib/libvirt/images/data-disk-${var.machine_name}${each.value["index"]}-xfs"
    }
  }
